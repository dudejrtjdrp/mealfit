...
export default function Premium() {
  return (
    <Screen scroll header={<StackHeader title="프리미엄 미리보기" />}>
      ...
      />
      <LockCard
        icon={<Ionicons name="calculator" size={22} color={colors.primary} />}
        title="커스텀 메뉴 계산 대행"
        desc="영양표가 없는 메뉴도 재료를 알려주시면 계산해 드려요."
        preview={['동네 김밥집 참치김밥 · 계산 요청', '회사 앞 샐러드 · 재료 사진 올리기']}
      />

      <Button title="출시되면 알려주세요" variant="outline" onPress={() => showToast('출시되면 알려드릴게요', 'info')} style={styles.cta} />
    </Screen>
  );
}
...
function LockCard({ icon, title, desc, preview }: { icon: ReactNode; title: string; desc: string; preview: string[] }) {
  return (
    <Card padding={20} style={styles.card}>
      ...
      <Text variant="body" color="ink2" style={styles.desc}>
        {desc}
      </Text>
...
const styles = StyleSheet.create({
  ...
  preview: { marginTop: spacing.md, backgroundColor: colors.surfaceAlt, borderRadius: radius.md, padding: spacing.md, gap: spacing.sm, overflow: 'hidden' },
  previewRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, opacity: 0.45 },
  previewDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: colors.gaugeFill },
  blur: {},
  lockOverlay: { position: 'absolute', right: spacing.md, top: spacing.md, flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: colors.primarySoft, borderRadius: radius.pill, paddingHorizontal: spacing.sm, paddingVertical: 2 },
