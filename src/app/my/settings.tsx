...
export default function Settings() {
  ...
  const doSignOut = async (kind: 'logout' | 'withdraw' | null = confirm) => {
    setConfirm(null);
    await signOut();
    showToast(kind === 'withdraw' ? '탈퇴했어요 · 이 기기의 정보를 지웠어요' : '로그아웃했어요', 'info');
  ...
  const ask = (kind: 'logout' | 'withdraw') => {
    ...
    Alert.alert(title, '이 기기에 저장된 프로필과 세션이 지워져요.', [
  ...
  return (
    <Screen scroll header={<StackHeader title="설정" />}>
      <Card padding={6} style={styles.card}>
        ...
        <ListRow
          title="데이터 저장 위치"
          subtitle={backend === 'local' ? '이 기기(로컬)' : '클라우드(Supabase)'}
          icon={<Ionicons name="phone-portrait-outline" size={22} color={colors.fat} />}
          iconBg={colors.fatBg}
          chevron={false}
          style={styles.row}
        />
      ...
      <Card padding={6} style={styles.card}>
        ...
        <View style={styles.sep} />
        <ListRow title="탈퇴하기" subtitle="이 기기의 프로필과 세션을 지워요." icon={<Ionicons name="person-remove-outline" size={22} color={colors.ink2} />} iconBg={colors.sodiumBg} onPress={() => ask('withdraw')} style={styles.row} />
      </Card>
      ...
      <BottomSheet
        ...
        subtitle="이 기기에 저장된 프로필과 세션이 지워져요."
        footer={
